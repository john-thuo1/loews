...
logger = setup_logger(__name__, 'graph_utils.log')
...
def plot_trend():
    try:
        ...
        monthly_tally = df.resample('ME', on='Report_Date')['Land_Size(Acres)'].sum().reset_index()
...
def plot_prophet_predictions(data):
    try:
        logger.info("Plotting Prophet predictions")
